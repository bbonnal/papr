...
void DrawArrowHead(M5Canvas& canvas, Vec2 tip, Vec2 from, double size)
{
  ...
  DrawLine(canvas, tip, p1);
  DrawLine(canvas, tip, p2);
}

void DrawArcBySegments(M5Canvas& canvas, Vec2 center, double radius, double startRad, double sweepRad, int steps)
{
  if (radius <= 0.01) {
    return;
  }
